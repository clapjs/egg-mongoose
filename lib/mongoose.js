...
module.exports = app => {
    app.Mongoose=app.mongoose = require('mongoose');
    app.addSingleton('clapMongoose', createOneClient);
};
...
async function createOneClient(config, app) {
    config=Object.assign({},{singleton:'model',include:true},config);
    ...
    const dirs=(Array.isArray(config.singletonSrc)?config.singletonSrc:[config.singletonSrc]).map(item=>path.join(app.baseDir, 'app/model', item));
    if(config.include){
        dirs.push(path.join(app.baseDir, 'node_modules/clap-mongoose-core/app/model'))
    }
